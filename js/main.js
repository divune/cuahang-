...
function changeInformation() {
    ...
    let inforphone = document.getElementById('inforphone');
    ...
    if (infoemail.value.length > 0) {     
        if (!emailIsValid(infoemail.value)) {
            document.querySelector('.inforemail-error').innerHTML = 'Email không đúng định dạng!';
            infoemail.value = '';
            toast({ title: 'Error', message: 'Cập nhật không thành công!', type: 'error', duration: 3000 });
        ...
        if(infophone.value.length != 10){
        if (/^\d{10}$/.test(phone)) { 
            toast({ title: 'Error', message: 'Cập nhật không thành công!', type: 'warning', duration: 4000}); 
        return;
        }else { 
            document.querySelector('.infophone-error').innerHTML = ''; 
            user.phone = infophone.value; 
        }
    }
        ...
        arrDonHang.forEach(item => {
            ...
            productHtml += `<div class="order-history-control">
                <div class="order-history-status">
                    ...
                    <button class="cancel-order-btn" onclick="cancelOrder('${item.id}')">
                    ✖ Hủy đơn</button>
                </div>
...
function cancelOrder(orderId) {
    ...
    let order = orderList.find(order => order.id === orderId && order.khachhang === currentUser.phone);
    if (!order) {
    ...
    // Lọc ra order không bị hủy
    let updatedOrders = orderList.filter(order => !(order.id === orderId && order.khachhang === currentUser.phone));
