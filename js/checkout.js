...
function thanhtoanpage(option,product) {
    ...
    function calculateShippingFee(total) {
        ...
        if (total >= 500000) return 15000;
        return 30000;
    }
    
    const totalCartValue = getCartTotal();
    const PHIVANCHUYEN = calculateShippingFee(totalCartValue);
    // Xu ly don hang
    switch (option) {
        case 1: // Truong hop thanh toan san pham trong gio
            ...
            <div class="price-detail">
                <span id="checkout-cart-total">${vnd(totalCartValue)}</span>
            </div>
        </div>
        <div class="priceFlx chk-ship">
            <div class="text">Phí vận chuyển</div>
            <div class="price-detail chk-free-ship">
                <span id="shipping-fee-display">${vnd(PHIVANCHUYEN)}</span>
            </div>
        </div>`;
            // Tong tien
            priceFinal.innerText = vnd(totalCartValue + PHIVANCHUYEN);
            break;
    ...
    tudenlay.addEventListener('click', () => {
        ...
        switch (option) {
            case 1:
                priceFinal.innerText = vnd(totalCartValue);
                break;
            case 2:
...
// Thông tin các đơn hàng đã mua - Xử lý khi nhấn nút đặt hàng
function xulyDathang(product) {
    ...
    if(tudenlay.classList.contains("active")){
        ...
        if(chinhanh1.checked) {
            diachinhan = "Hòa Khánh Nam, Đà Nẵng";
        }
        if(chinhanh2.checked) {
            diachinhan = "312 Điện biên Phủ, Đà Nẵng";
        }
    ...
    if (!/^\d{10}$/.test(sdtnhan)) {
        toast({ title: 'Chú ý', message: 'Số điện thoại phải đúng định dạng 10 chữ số !', type: 'warning', duration: 4000 });
        return;
    }
